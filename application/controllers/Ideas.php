...
class Ideas extends CI_Controller {
    ...
    public function index() {
        ...
        $datosa= $this->ideas_model->conteoIdeasCerradas($this->session->userdata('id_usuario'));
        $gest = false;
        if($datosa!=FALSE){
        foreach ($datosa->result() as $listaideas) {
                 $gest = $listaideas->id_idea;
            }
        }
        $datos['conteoideasgest'] = $gest;
        $this->load->helper('url');
        ...
        foreach ($fases->result() as $listfases) {
            ...
            if ($actividades != false) {
                ...
                foreach ($actividades->result() as $listactividades) {
                    $contactividades++;
                    $entregablesporact = $this->reportes_model->obtenerDatosEntregable($listactividades->id_actividad, $codIdea, $parame);
                    $conteoentregable = 0;
                    $conteoaprobadas = 0;
    ...
    public function guardarVersion() {
        ...
        $this->session->set_flashdata('correcto', 'Versión Guardada Correctamente!');
    ...
    public function solicitarRevision() {
        ...
        $this->session->set_flashdata('correcto', 'Versión Enviada Correctamente!');
        redirect(base_url() . 'ideas/historiales/' . $idea . '/' . $entregable, 'refresh');
    }
